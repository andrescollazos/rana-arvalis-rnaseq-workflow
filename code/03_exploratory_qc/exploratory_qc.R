...
text(x = par("usr")[2], y = median_reads,
     labels = paste0("Median (", round(median_reads, 2), "M)"),
     pos = 2, cex = 0.8)

text(x = par("usr")[2], y = half_median_reads,
     labels = paste0("50% Median (", round(half_median_reads, 2), "M)"),
     pos = 2, cex = 0.8)

dev.off()

#region Dataset and Low-count filtering
library(DESeq2)
...
#region Log-count distribution before vs after filtering
