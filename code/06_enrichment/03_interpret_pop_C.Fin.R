...
df_up <- ora_tables_BP_sig[["C.Fin_full_up"]]
df_down <- ora_tables_BP_sig[["C.Fin_full_down"]]
...
# 8.1 Treemap — C.Fin_full_down (main plot)
pdf(file = "BP/treemap_C.Fin_full_down_BP_sig.pdf")
treemapPlot(reduced_down)
dev.off()

# 8.2 Treemap — C.Fin_full_up (check redundancy)
pdf(file = "BP/treemap_C.Fin_full_up_BP_sig.pdf")
treemapPlot(reduced_up)
dev.off()

# 8.3 Scatterplot — C.Fin_full_down
pdf(file = "BP/scatter_C.Fin_full_down_BP_sig.pdf")
scatterPlot(
    simMatrix_down,
    reduced_down
)
...
# 8.4 Scatterplot — C.Fin_full_up
pdf(file = "BP/scatter_C.Fin_full_up_BP_sig.pdf")
scatterPlot(
    simMatrix_up,
    reduced_up
)
...
pdf(file = "BP/dotplots_C.Fin_full_down_BP_sig.pdf")
## C.Fin_full_down
res_cd <- ora_results_BP[["C.Fin_full_down"]]
...
## C.Fin_full_up
res_cu <- ora_results_BP[["C.Fin_full_up"]]
...
save.image("03_interpret_pop_C.Fin.RData")
